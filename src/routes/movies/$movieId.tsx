...
import { Star, Clock, CalendarDays, PlayCircle, Users } from "lucide-react";
import { getMovie, listShowtimes } from "@/lib/movies.functions";
...
export const Route = createFileRoute("/movies/$movieId")({
  loader: ({ context, params }) =>
    context.queryClient.ensureQueryData(movieQuery(params.movieId)),
  head: ({ loaderData }) => ({
    meta: [
...
function MovieDetailPage() {
  ...
  const byTheatre = new Map<string, typeof visibleShows>();
  for (const show of visibleShows) {
    const key = `${show.theatre_name} · ${show.city}`;
    byTheatre.set(key, [...(byTheatre.get(key) ?? []), show]);
  }
      ...
      {movie.status === "now_showing" && (
        <section className="mx-auto max-w-6xl px-4 pb-16">
          ...
          {showsLoading ? (
            ...
            <div className="space-y-4">
              {[...byTheatre.entries()].map(([theatre, shows]) => (
                <div key={theatre} className="rounded-xl border border-border bg-card p-4">
                  <p className="mb-3 text-sm font-semibold text-card-foreground">{theatre}</p>
